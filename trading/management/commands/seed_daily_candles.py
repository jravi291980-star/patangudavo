...
# Key where websocket.py looks for the fixed SMA
FIXED_SMA_KEY = "algo:fixed_vol_sma"
FIXED_SMA_DIVISOR = 1875  # 5 Days * 375 Minutes

class Command(BaseCommand):
    help = 'Fetches last 5 days of DAILY volume, calculates Fixed SMA (Vol/1875), and seeds Redis.'

    def _get_start_date(self, days_back=7):
        """
        Finds the start date going back N days to ensure we get at least 5 trading days.
        Defaults to 7 calendar days to be safe against weekends.
        """
        today = dt.now(IST).date()
    ...
    def fetch_and_calculate_sma(self, kite, symbol, token, from_date, to_date):
        """
        Fetches DAILY candles, sums volume, divides by 1875.
        """
        # Fetch DAILY data
        candles = kite.historical_data(
            instrument_token=token,
        ...
        # Sort by date descending and take the last 5 COMPLETED days
        # We exclude today if it's in the list (as today is incomplete)
        # However, historical_data usually returns up to yesterday if run before market close? 
        # Safest: Just take the last 5 available records.
        
        valid_candles = candles[-5:] # Take last 5 days
        
        if len(valid_candles) < 1:
            return None

        total_volume = sum([c['volume'] for c in valid_candles])
        
        # Formula: Average Volume per Minute over 5 days
        # Since 1875 = 375 mins * 5 days, this is effectively the 5-day SMA of minute volume
        fixed_sma = int(total_volume / FIXED_SMA_DIVISOR)
    ...
    def handle(self, *args, **options):
        ...
        # 1. Date Range
        start_date = self._get_start_date(days_back=10) # Safe buffer
        end_date = dt.now(IST).date() # Up to today
        
        self.stdout.write(self.style.SUCCESS(f"Fetching Daily Data from {start_date} to {end_date}"))
        ...
        for symbol, token in symbol_token_map.items():
            try:
                sma = self.fetch_and_calculate_sma(kite, symbol, token, start_date, end_date)
                if sma is not None:
                    sma_map[symbol] = sma
            except Exception as e:
                logger.error(f"SMA Calc Error {symbol}: {e}")
            
            processed += 1
            if processed % 50 == 0:
                self.stdout.write(f"Processed: {processed}/{total_stocks}")
            time.sleep(0.1) # Rate limit protection

        # 2. Bulk Save to Redis
        if sma_map:
            try:
                # Store in Hash used by websocket.py
                redis_client.delete(FIXED_SMA_KEY)
                redis_client.hset(FIXED_SMA_KEY, mapping=sma_map)
                self.stdout.write(self.style.SUCCESS(f"Successfully seeded Fixed SMA for {len(sma_map)} stocks."))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Redis Write Error: {e}"))
        else:
            self.stdout.write(self.style.WARNING("No data calculated."))