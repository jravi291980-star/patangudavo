...
# model 3 
"""
Refactored Websocket - MEMORY OPTIMIZED (No Historical Lists)
- LOGIC: Triggers via candle_1m Stream, Exits via live_ohlc_data.
- OPTIMIZATION: Removed candles:1m:{symbol} lists to save ~90% Redis RAM.
- PERSISTENCE: Retains first_candle_915 for Momentum engines.
"""
...
def cache_instruments_for_day():
    ...
    try:
        ...
        for inst in nse_instruments:
            ...
            full_symbol = f"{exchange}:{symbol}"
            instrument_data = {
                'symbol': symbol, 'exchange': exchange, 'instrument_type': 'EQ',
                'lot_size': inst.get('lot_size', 1), 'tick_size': inst.get('tick_size', 0.05),
                'open': 0.0, 'high': 0.0, 'low': 0.0, 'close': 0.0, 'ltp': 0.0, 'change': 0.0, 'volume': 0,
            }
        ...
        pipe.set('circuit_limits', json.dumps(circuit_limits, default=_default_json_serializer))
        pipe.execute()
        logger.info(f"WEBSOCKET_CACHE: Successfully cached {len(instrument_map)} instruments.")
    except Exception as e:
        logger.error(f"WEBSOCKET_CACHE: Failed to cache instruments: {e}", exc_info=True)

# --- CANDLE AGGREGATOR (REFACTORED FOR RAM) ---

class CandleAggregator:
    def __init__(self, redis_client, filter_criteria, qualified_set_ref, whitelist_ref):
        self.redis = redis_client
        self.active_candle = {}
        self.last_total_volume = {}
        self.lock = threading.Lock()
    ...
    def _is_trading_minute(self, ts: dt) -> bool:
        t = ts.astimezone(IST).time()
        return (t >= datetime_time(9, 15)) and (t < datetime_time(15, 30))
    ...
    def is_qualified(self, symbol, vol, close):
        ...
        sma = self.fixed_sma_map.get(symbol, 0)
        if close <= 0 or sma <= 0: return False
        ...
        for criteria in self.filter_criteria:
            try:
                min_sma = criteria.get('min_sma', 0)
                if sma < min_sma: continue
                mult = criteria.get('mult', 0)
                if vol < (sma * mult): continue
                min_vp = criteria.get('min_vp', 0)
                if vol_price_cr >= min_vp:
                    self.qualified_symbols.add(symbol)
                    return True
    ...
    def process_tick(self, symbol, ltp, ts, cumulative_volume):
        ...
        with self.lock:
            ...
            # Optimized Live Snapshot (One key per active stock)
            try:
                cur = self.active_candle[symbol]
                if symbol in self.qualified_symbols or symbol in self.whitelist_ref:
                    payload = {'ts': cur['bucket'].isoformat(), 'open': cur['open'], 'high': cur['high'], 'low': cur['low'], 'close': cur['close'], 'volume': cur['volume']}
                    self.redis.set(f'current_1m_candle:{symbol}', json.dumps(payload, default=_default_json_serializer), ex=120)
            except: pass
    ...
    def _publish_and_store(self, symbol, candle):
        ...
        if not self.is_qualified(symbol, vol, close): return 
        ...
        try:
            # --- MEMORY OPTIMIZATION: NO LISTS (RPUSH/LTRIM REMOVED) ---
            
            # Keep 9:15 AM Logic for Momentum Engines (Vital for strategy)
            if candle['bucket'].time() == datetime_time(9, 15):
                self.redis.set(f"{FIRST_CANDLE_PREFIX}:{symbol}", json.dumps(payload, default=_default_json_serializer), ex=86400)

            # Push to the Live Stream (Capped)
            self.redis.xadd(CANDLE_STREAM_KEY, {'data': json.dumps(payload, default=_default_json_serializer)}, maxlen=1000, approximate=True)
...
class LiveTickerManager:
    def __init__(self, api_key, access_token, account_id):
        ...
        # Memory Optimized Candle Aggregator
        self.candle_agg = CandleAggregator(self.redis_client, self.filter_criteria, self.qualified_symbols, self.whitelist)

        self.kws.on_ticks = self.on_ticks
        self.kws.on_connect = self.on_connect
        self.kws.on_close = self.on_close
        self.kws.on_error = self.on_error
        self.last_sma_reload_date = None

    def _load_strategy_criteria(self):
        criteria = []
        try:
            for key in [KEY_BULL_SETTINGS, KEY_BEAR_SETTINGS]:
                raw = self.redis_client.get(key)
                if raw:
                    data = json.loads(raw).get('volume_criteria', [])
                    for c in data: criteria.append({'min_vp': float(c.get('min_vol_price_cr', 0)), 'min_sma': float(c.get('min_sma_avg', 0)), 'mult': float(c.get('sma_multiplier', 0))})
            if not criteria:
                criteria.append({'min_vp': 5.0, 'min_sma': 50000, 'mult': 2.0})
        except: 
            criteria.append({'min_vp': 5.0, 'min_sma': 50000, 'mult': 2.0})
        return criteria

    def _refresh_whitelist(self):
        if time.time() - self.last_whitelist_update < 5: return 
        try:
            keys = [f"cb:active:{self.account_id}", f"cbd:active:{self.account_id}", 
                    f"mom_bull_active_entries:{self.account_id}", f"mom_bear_active_entries:{self.account_id}"]
            active_symbols = self.redis_client.sunion(keys)
            self.whitelist = {s.decode() if isinstance(s, bytes) else s for s in active_symbols} if active_symbols else set()
            self.last_whitelist_update = time.time()
        except: pass

    def _load_instruments_from_cache(self) -> bool:
        try:
            raw = self.redis_client.get('instrument_map')
            if raw:
                full_map = json.loads(raw.decode() if isinstance(raw, bytes) else raw)
                self.instrument_map = {k: v for k, v in full_map.items() if v.get('exchange') == 'NSE' or v.get('symbol') in settings.INDEX_SYMBOLS}
                return True
            return False
        except Exception: return False
    ...
    def on_ticks(self, ws, ticks):
        ...
        for tick in ticks:
            token_str = str(tick.get('instrument_token') or tick.get('token'))
            if token_str not in self.instrument_map: continue
            
            inst = self.instrument_map[token_str]
            ltp = float(tick.get('last_price', 0) or inst.get('ltp', 0))
            inst['ltp'] = ltp
            
            # Simple OHLC update logic...
            inst['volume'] = int(tick.get('volume_traded') or tick.get('total_traded_quantity') or inst.get('volume', 0))
            sym = inst.get('symbol')

            if sym and inst.get('exchange') == 'NSE':
                ts = tick.get('timestamp', dt.now(IST))
                self.candle_agg.process_tick(sym, ltp, ts, inst['volume'])
                
                # Filtered Tick Stream (Only for active or qualified stocks)
                if sym in self.qualified_symbols or sym in self.whitelist:
                    stream_data = {'symbol': sym, 'ltp': str(ltp), 'volume': str(inst['volume']), 'ts': dt.now(IST).isoformat()}
                    pipe.xadd(TICK_STREAM_KEY, stream_data, maxlen=5000, approximate=True)
                    has_tick_data = True

        if has_tick_data: pipe.execute()
    ...
    def publish_to_redis(self):
        try:
            ...
            for token, data in self.instrument_map.items():
                sym = data.get('symbol')
                if not sym or data.get('ltp', 0) <= 0: continue
                if sym in settings.INDEX_SYMBOLS or sym in self.qualified_symbols or sym in self.whitelist:
                    live_ohlc[sym] = {
                        'ltp': data.get('ltp', 0), 'open': data.get('open', 0), 'high': data.get('high', 0),
                        'low': data.get('low', 0), 'close': data.get('close', 0), 'volume': int(data.get('volume', 0)),
                    }
            if live_ohlc:
                self.redis_client.set('live_ohlc_data', json.dumps(live_ohlc, default=_default_json_serializer))
        except: pass

    def on_connect(self, ws, response):
        logger.info("WEBSOCKET: Connected.")
        subs = [safe_int(t) for t, d in self.instrument_map.items()]
        self.subscribed_tokens = set(subs)
        if subs: ws.subscribe(subs); ws.set_mode(ws.MODE_FULL, subs)

    def on_close(self, ws, code, reason): logger.warning(f"WEBSOCKET: Closed {code}")
    def on_error(self, ws, code, reason): logger.error(f"WEBSOCKET: Error {code}")

    @retry(stop=stop_after_attempt(5), wait=wait_exponential(multiplier=1, min=2, max=10))
    def start_connection(self): self.kws.connect(threaded=True)

    def run(self):
        self._load_instruments_from_cache()
        while self.running:
            now = dt.now(IST); today = now.date()
            if now.time() >= datetime_time(9, 10) and self.last_sma_reload_date != today:
                self.candle_agg.load_fixed_sma(); self.last_sma_reload_date = today

            if datetime_time(9, 10) <= now.time() < datetime_time(15, 35):
                if not getattr(self.kws, 'is_connected', lambda: False)(): self.start_connection()
            else:
                if getattr(self.kws, 'is_connected', lambda: False)(): self.kws.close()
            time.sleep(30)

    def stop(self): self.running = False; self.candle_agg.flush_all()
