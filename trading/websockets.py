...
# --- Main Ticker Management Class ---

class CandleAggregator:
    """
    Manages 1-minute candle creation, storage, and derived metrics 
    like Volume SMA and multiplier.
    """
    def __init__(self, redis_instance):
        self.redis = redis_instance
        self.active_candle = {}
        self.last_total_volume = {}
        self.sma_period = 375  # One full trading day (9:15 to 3:30)
        self.max_candles = MAX_CANDLES_TO_STORE # 400 or more to allow trimming
        self.is_market_open = False
        
    def _get_key(self, symbol):
        return f'candles:1m:{symbol}'

    def _calculate_sma_and_vol_stats(self, symbol, current_volume):
        """
        Calculates 375-period Volume SMA using only closed 1-min candles 
        from Redis and determines the volume multiplier.
        """
        key = self._get_key(symbol)
        
        # Fetch the last 375 closed candles (excluding the current developing one)
        prev_candles_raw = self.redis.lrange(key, -self.sma_period, -1)
        ...
        for raw in prev_candles_raw:
            try:
                c_data = json.loads(raw)
                # CRITICAL: We append the DISCRETE 1-minute volume
                volumes.append(c_data.get('volume', 0))
            except Exception as e:
        ...
        # Ensure we only calculate SMA if we have enough data
        if len(volumes) >= self.sma_period:
            # We have a full set of 375 closed candles (either historical or real-time)
            sma_val = sum(volumes) / len(volumes)
            logger.debug(f"{symbol}: Full 375 SMA calculated: {sma_val:.2f}")
        elif volumes:
            # We have some data (less than 375, likely at the very start of backfill)
            sma_val = sum(volumes) / len(volumes)
            logger.warning(f"{symbol}: Partial SMA (Count: {len(volumes)}) calculated: {sma_val:.2f}")
        else:
            # Cold start / no history found at all. Use current volume as fallback.
            sma_val = current_volume if current_volume else 1.0
            logger.warning(f"{symbol}: COLD START - Using current volume {sma_val} as SMA base.")


        volume_multiplier = current_volume / sma_val if sma_val > 0 else 0
        
        return sma_val, volume_multiplier

    def _publish_and_store(self, symbol, candle):
        """
        Finalizes a closed candle, stores it in Redis, trims the list, 
        and calculates SMA metrics.
        """
        key = self._get_key(symbol)
        
        # 1. Calculate SMA and Multiplier *before* storing the new candle
        # We use the current candle's volume for comparison
        candle_volume = candle.get('volume', 0)
        sma_val, volume_multiplier = self._calculate_sma_and_vol_stats(symbol, candle_volume)
        
        # Add metrics to the candle payload
        candle['sma_volume_375'] = round(sma_val, 2)
        candle['volume_multiplier'] = round(volume_multiplier, 2)
        
        # 2. Store the closed candle in Redis (JSON format)
        payload_str = json.dumps(candle, default=_default_json_serializer)

        pipe = self.redis.pipeline()
        pipe.rpush(key, payload_str)
        # 3. Trim the list to ensure it never exceeds max_candles
        # This automatically removes the oldest candle as new ones are added
        pipe.ltrim(key, -self.max_candles, -1)
        pipe.execute()

        logger.info(
            f"CLOSED: {symbol} @ {candle['ts']}. V: {candle['volume']} | "
            f"SMA(375): {sma_val:.2f} | Multiplier: {volume_multiplier:.2f}"
        )

        # 4. TODO: Publish to other services (e.g., strategy engine)
        # self.redis.publish('closed_candles', payload_str)


    def process_tick(self, tick, instrument_map):
        """
        Processes a live tick to aggregate 1-minute candles.
        """
        ...
        symbol = instrument_map[token]['symbol']
        key_active = f'active:{symbol}'
        
        # Get current time in IST
        now = dt.now(IST)
        current_minute_ts = now.replace(second=0, microsecond=0)
        
        # --- Volume Calculation (Converting Cumulative to Discrete) ---
        ...
        if cumulative_volume is not None:
            ...
            # Calculate volume since the last tick
            if prev_total is None:
                delta_vol = cur_total
            else:
                delta_vol = max(0, cur_total - prev_total) 
        ...
        current_candle = self.active_candle.get(key_active)

        if not current_candle or current_candle['ts'] != current_minute_ts.isoformat():
            # New minute started: Close the old candle and start a new one

            if current_candle:
                # The old candle is complete and must be stored/processed
                self._publish_and_store(symbol, current_candle)
            ...
            self.active_candle[key_active] = {
                'symbol': symbol,
                'interval': '1m',
                'ts': current_minute_ts.isoformat(),
                'open': ltp,
                'high': ltp,
                'low': ltp,
                'close': ltp,
            ...
            # Update the existing candle within the current minute
            c = current_candle
            ltp = tick.get('last_price', c['close']) # Use previous close as fallback
    ...
    def get_last_trading_day(self, date_obj):
        """Finds the last trading day."""
        date_obj = date_obj.date() if isinstance(date_obj, dt) else date_obj
        # Start checking from yesterday
        date_obj -= timedelta(days=1)
        # Loop backwards until a weekday (Mon=0, Fri=4)
        while date_obj.weekday() >= 5: # Saturday or Sunday
            date_obj -= timedelta(days=1)
        return date_obj
        
    def backfill_history(self, kite, instrument_map):
        """
        Fetches CLOSED historical data for the last one or two trading days 
        to seed Redis and satisfy the 375-candle SMA requirement.
        """
        logger.info("BACKFILL: Starting historical data warmup for 375-min SMA...")
        
        # 1. Define the necessary date range for CLOSED data
        # We primarily target the last full trading day (yesterday)
        current_date = dt.now(IST)
        
        # Get the date of the previous trading day
        yesterday_date = self.get_last_trading_day(current_date)
        
        # If we need more data than yesterday provides (375 candles)
        # we can fetch the day before as well.
        day_before_date = self.get_last_trading_day(yesterday_date)

        # 2. Filter target instruments (NSE stocks/indices)
        targets = []
        for token, data in instrument_map.items():
            if data['symbol'] in STOCK_INDEX_MAPPING and data['exchange'] == 'NSE':
                targets.append((token, data['symbol']))

        count = 0
        for token, symbol in targets:
            key = self._get_key(symbol)
            
            # Check if Redis already has sufficient data (e.g., server was running yesterday)
            # We skip if we have at least 375 candles already stored
            if self.redis.llen(key) >= self.sma_period: 
                continue

            history_candles = []
            fetch_dates = [yesterday_date, day_before_date]
            
            # Fetch data from oldest to newest (to ensure correct list order)
            for fetch_date in reversed(fetch_dates): 
                # Skip if the fetch date is today or later
                if fetch_date >= current_date.date():
                    continue

                logger.debug(f"BACKFILL: Fetching {symbol} history for {fetch_date}")
                try:
                    # Fetch history from Kite for a full day
                    # Note: Kite API works best for full days, not partial current-day data
                    ...
                    history_candles.extend(history)
                except Exception as e:
                    # Log rate limit or other API errors but continue with other symbols
                    logger.warning(f"BACKFILL: API Failed for {symbol} on {fetch_date}: {e}")
                    time.sleep(0.1) 
            
            # 3. Process and store the fetched history
            if history_candles:
                pipe = self.redis.pipeline()
                pipe.delete(key) # Clear any old, incomplete data before repopulating
                
                # We only need the last 375 candles for efficiency
                history_candles = history_candles[-self.max_candles:]

                for candle in history_candles:
                    # Ensure candle data is correctly formatted
                    ts = candle['date']
                    ...
                    payload = {
                        ...
                        'low': candle['low'],
                        'close': candle['close'],
                        # Kite historical data returns discrete volume already
                        'volume': candle['volume'] 
                    }
                    pipe.rpush(key, json.dumps(payload, default=_default_json_serializer))
                
                # Trim immediately after backfill to maintain size
                pipe.ltrim(key, -self.max_candles, -1)
                pipe.execute()
                count += 1
                logger.debug(f"BACKFILL: {symbol} seeded with {len(history_candles)} candles.")
            
        logger.info(f"BACKFILL: Successfully warmed up data for {count} symbols.")

# --- Main Ticker Manager ---

class LiveTickerManager:
    """
    Manages the KiteTicker connection and distributes ticks for aggregation.
    """
    def __init__(self, api_key, access_token):
        ...
        self.candle_agg = CandleAggregator(redis_client)
        self.ticker = None
        self.running = False
        
        self._load_instruments_from_cache()

    def _load_instruments_from_cache(self):
        """Load instruments and setup subscriptions."""
        try:
            kite = get_kite(self)
            self.instrument_map = get_instrument_map(kite)
            
            self.subscribed_tokens = []
            for token, data in self.instrument_map.items():
                if data['symbol'] in STOCK_INDEX_MAPPING:
                    self.subscribed_tokens.append(int(token))

            logger.info(f"MANAGER: Loaded {len(self.instrument_map)} instruments. Subscribing to {len(self.subscribed_tokens)} tokens.")
        
        except Exception as e:
            logger.error(f"MANAGER: Failed to load instruments: {e}")
            sys.exit(1)
    ...
    def on_tick(self, ws, ticks):
        ...
        for tick in ticks:
            self.candle_agg.process_tick(tick, self.instrument_map)

    def on_connect(self, ws, response):
        """Callback on successful connection."""
        ws.subscribe(self.subscribed_tokens)
        ws.set_mode(ws.MODE_FULL, self.subscribed_tokens)
        logger.info(f"WEBSOCKET: Subscribed to {len(self.subscribed_tokens)} tokens in FULL mode.")
    ...
    def on_reconnect(self, ws, attempts):
        """Callback on automatic reconnection attempt."""
        logger.warning(f"WEBSOCKET: Reconnecting... Attempt {attempts}")
    
    def on_noreconnect(self, ws):
        """Callback when reconnection attempts exhaust."""
        logger.error("WEBSOCKET: Reconnection attempts exhausted.")
        self.running = False

    def run(self):
        """Starts the Kite Ticker loop."""
        self.running = True
        while self.running:
            try:
                ...
                logger.info("WEBSOCKET: Starting connection loop...")
                self.ticker.connect(threaded=False, disable_ssl_check=True)
                
            except TokenException:
                logger.error("WEBSOCKET: Token error. Authentication failed.")
                time.sleep(30) # Wait before retry
            except Exception as e:
                logger.error(f"WEBSOCKET: Unhandled error in run loop: {e}", exc_info=True)
                time.sleep(5)
            
            if self.running and not self.ticker.is_connected():
                logger.info("WEBSOCKET: Ticker connection lost, restarting loop in 10 seconds...")
                time.sleep(10)
        logger.info("WEBSOCKET: Ticker manager stopped.")
...
def start_websocket_thread(master_account):
    if not redis_client: 
        logger.error("WEBSOCKET: Cannot start, Redis client is not available.")
        return
    ...
    try:
        # We need a KiteConnect instance to fetch history
        kite = get_kite(master_account)
            
        # Run Backfill synchronously before starting the websocket thread
        # This ensures SMA is ready before the first live tick arrives
        # This backfill uses YESTERDAY's data to be robust against current-day API lag.
        ticker_manager.candle_agg.backfill_history(kite, ticker_manager.instrument_map)
        
    except Exception as e:
        # This is non-fatal; the application can still run but with cold-start SMA
        logger.error(f"WEBSOCKET: Critical Backfill failed on startup: {e}", exc_info=True)
