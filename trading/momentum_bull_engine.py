"""
Momentum Bull Engine (1-Min First Candle Breakout - LONG)
...
from trading.models import Account, MomentumBullTrade
...
KEY_SETTINGS = "algo:settings:mom_bull"
KEY_BLACKLIST = "algo:blacklist"
KEY_ENGINE_ENABLED = "algo:engine:mom_bull:enabled"
KEY_PANIC_TRIGGER = "algo:panic:mom_bull"

# --- CONSTANTS ---
SL_BUFFER_PCT = 0.0002
ENTRY_BUFFER_PCT = 0.0001
GAP_LIMIT_PCT = 0.03
MAX_MONITORING_MINUTES = 45 # Stop monitoring if no breakout in first 45 mins

class MomentumBullClient:
    ...
    def __init__(self, account: Account):
        ...
        # Identifiers
        self.group_name = f"MB_GROUP:{self.account.id}"
        self.consumer_name = f"MB_CONSUMER:{threading.get_ident()}"
        ...
        self.trade_count_key = f"mom_bull_count:{self.account.id}:{today_iso}"
        self.active_entries_set = f"mom_bull_active_entries:{self.account.id}"
        self.exiting_trades_set = f"mom_bull_exiting:{self.account.id}"
        self.lock_prefix = f"mom_bull_lock:{self.account.id}"
        self.daily_pnl_key = f"mom_bull_pnl:{self.account.id}:{today_iso}"
        self.force_exit_set = f"mom_bull_force_exit_requests:{self.account.id}"
        ...
        if not redis_client: 
            logger.error("MB: Redis unavailable")
            self.running = False
            return
    ...
    def _ensure_consumer_group(self, stream_key, start_id='$'):
        try:
            ...
            if "BUSYGROUP" not in str(e): logger.error(f"MB: Group Create Error: {e}")
        except Exception: pass

    def _prefill_prices(self):
        try:
            raw = redis_client.get('live_ohlc_data') # Standard key from websocket.py
            if raw:
                data = json.loads(raw)
                for sym, info in data.items():
                    self.latest_prices[sym] = float(info.get('ltp', 0))
            logger.info(f"MB: Prefilled prices for {len(self.latest_prices)} symbols.")
        except Exception: pass
    ...
    def _daily_reset(self):
        ...
        if now.time() >= self.DAILY_RESET_TIME:
            reset_key = f"mom_bull_reset:{now.date()}"
            if redis_client.set(reset_key, "1", nx=True, ex=86400):
                ...
                logger.info("MB: Daily Reset Triggered.")

    def _load_state_from_db(self):
        try:
            close_old_connections()
            today_start = IST.localize(dt.combine(dt.now(IST).date(), dt_time.min))
            qs = MomentumBullTrade.objects.filter(account=self.account, created_at__gte=today_start, status__in=['OPEN', 'PENDING_ENTRY', 'PENDING_EXIT'])
            ...
            # Count excludes failed/expired for limit check
            count = MomentumBullTrade.objects.filter(account=self.account, created_at__gte=today_start).exclude(status__in=['FAILED_ENTRY', 'EXPIRED']).count()
    ...
    def scan_for_valid_setups(self):
        now = dt.now(IST).time()
        # Only scan between 9:16 and 9:17 (After first candle closes)
        if not (dt_time(9, 16) <= now < dt_time(9, 17)):
            # Cleanup monitoring list later in the day
        ...
        for symbol in active_symbols_keys:
            ...
            else: passed_vol = True # Default pass if no criteria
            
            if not passed_vol: continue

            # 5. Redis Check (Prev Day)
            ...
            # 6. Gap Check
            if ((c1['high'] - prev_close) / prev_close) > GAP_LIMIT_PCT: continue
            
            self.monitoring_list.add(symbol)
            logger.info(f"MB: Added {symbol} to monitoring list.")

    # --- CORE LOGIC: MONITOR BREAKOUT ---
    def monitor_breakouts(self):
        # Allow monitoring for extended period
        now = dt.now(IST).time()
        if now < dt_time(9, 16): return # Wait for first candle to complete
        ...
        for symbol in list(self.monitoring_list):
            ...
            # Check Breakout
            if ltp > (c1['high'] * (1.0 + ENTRY_BUFFER_PCT)):
                raw_pd = redis_client.hget(PREV_DAY_HASH, symbol)
                pc = float(json.loads(raw_pd).get('close', 0)) if raw_pd else 0
                self._attempt_entry(symbol, ltp, c1['high'], pc)
                self.monitoring_list.discard(symbol) # Stop monitoring once triggered

    def _attempt_entry(self, symbol, entry_price, c1_high, prev_close):
        ...
        try:
            ...
            try:
                order_id = self.kite.place_order(tradingsymbol=symbol, exchange='NSE', transaction_type='BUY', quantity=qty, order_type='MARKET', product='MIS', variety='regular')
            except Exception as e:
                logger.error(f"MB: Order Failed {symbol}: {e}")
                self._rollback_trade_count(); return

            with transaction.atomic():
                t = MomentumBullTrade.objects.create(
                    ...
                    entry_time=dt.now(IST), first_candle_high=c1_high, 
                ...
                logger.info(f"MB: BUY Executed {symbol} Qty: {qty}")
        except Exception as e:
            logger.error(f"MB: Setup DB Error {symbol}: {e}")
    ...
    def manage_active_trades(self):
        ...
        for symbol, trade in list(self.open_trades.items()):
            ...
            self._smart_log(symbol, f"MB: Monitor {symbol} PnL:{ltp - trade.entry_price:.2f}")

            # 2. Trailing SL
            rr_str = sett.get('risk_reward', '1:2')
            rr = float(rr_str.split(':')[1]) if ':' in rr_str else 2.0
            init_risk = (trade.target_level - trade.entry_price) / rr
            
            if init_risk > 0:
                step_size = init_risk * trail_ratio
                curr_profit = ltp - trade.entry_price
                if curr_profit >= step_size:
                    ...
                    if new_sl > trade.stop_level:
                        ...
                        logger.info(f"MB: Trailed SL {symbol} to {new_sl}")
    ...
    def _initiate_exit(self, trade, reason):
        ...
        try:
            ...
            logger.info(f"MB: EXIT Initiated {trade.symbol} ({reason})")
        except Exception as e:
            logger.error(f"MB: Exit Failed {trade.symbol}: {e}")
            redis_client.srem(self.exiting_trades_set, trade.id)

    def _panic_exit_all(self):
        logger.warning("MB: PANIC EXIT TRIGGERED!")
    ...
    def _listen_to_tick_stream(self):
        ...
        while self.running:
            ...
            try:
                ...
                logger.error(f"MB Stream Error: {e}")
                time.sleep(1)
    ...
    def run(self):
        self._daily_reset()
        t_data = threading.Thread(target=self._listen_to_tick_stream, daemon=True, name="MB_Data")
        t_data.start()
        threading.Thread(target=self._reconcile_loop, daemon=True, name="MB_Reconcile").start()

        logger.info("MomentumBull: Engine Started.")

        while self.running:
            try:
                # HEARTBEAT CHECK
                if time.time() - self.last_data_beat > 15:
                    logger.critical("MB: DATA THREAD DIED! Restarting Engine...")
                ...
                logger.error(f"MB Main Loop Error: {e}")
                time.sleep(1)
